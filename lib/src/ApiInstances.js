...
class Apis {
    ...
    */
    static reset( cs = "ws://localhost:8090", connect, connectTimeout = 4000, optionalApis,rpc_user,rpc_password, closeCb ) {
    ...
    static instance( cs = "ws://localhost:8090", connect, connectTimeout = 4000, optionalApis, rpc_user,rpc_password,closeCb) {
        if ( ! inst ) {
            inst = new Apis();
            inst.setRpcConnectionStatusCallback(this.statusCb);
        }
    ...
    static asset = new Proxy(Apis, {
        get(apis, method) {
            return function() {
                return apis.instance().asset_api().exec(method,[...arguments])
            }
        }
    })

    /** @arg {string} connection .. */
    connect( cs, connectTimeout, optionalApis = {enableCrypto: false, enableOrders: false,enableAsset:false},rpc_user,rpc_password ) {
        ...
        this.init_promise = this.ws_rpc.login(rpc_user, rpc_password).then(() => {
            ...
            if (optionalApis.enableAsset) this._asset = new GrapheneApi(this.ws_rpc, "asset");
            ...
            this.ws_rpc.on_reconnect = () => {
                ...
                this.ws_rpc.login("", "").then(() => {
                    ...
                    if (optionalApis.enableAsset) this._asset.init();
                });
            }
            ...
            if (optionalApis.enableAsset) initPromises.push(this._asset.init());
